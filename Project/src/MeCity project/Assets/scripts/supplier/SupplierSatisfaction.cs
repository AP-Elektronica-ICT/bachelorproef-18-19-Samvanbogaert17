...
public class SupplierSatisfaction : MonoBehaviour
{
            ...
            if (popUps[i] == null)
            {
                ...
                popUps[i].Prefab.GetComponentsInChildren<Button>()[0].onClick.AddListener(() =>
                {
                    ...
                    if (!CameraControl.showingPopUp)
                    {
                        ...
                        Destroy(popUps[i].Prefab);
                        popUps[i] = null;
                        FindObjectOfType<EventSystem>().Task();
        ...
        for (int i = 0; i < popUps.Length; i++)
        {
            if (popUps[i] == null)
            {
                ...
                popUps[i].Prefab.GetComponentsInChildren<Button>()[0].onClick.AddListener(() =>
                {
                    if (!CameraControl.showingPopUp)
                    {
                        moneyText.text = string.Format("{0:n}",(float.Parse(moneyText.text) + calculatedPrice));
                        totalSpentEnergy = 0;
                        Destroy(popUps[i].Prefab);
                        popUps[i] = null;
                    }
